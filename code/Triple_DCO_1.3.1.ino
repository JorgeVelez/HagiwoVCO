...
int Mode = 0;
int Mode_CV = 0;
int ModeFromButton = 0;      // mode state before adding CV
...
void updateControl() {
  ...
  // Push button is on D5 (pin 8), pull up, low on press
  // (D4 is unused)
  ...
  // This part still isn't working quite right - CV only toggles between two modes, not all three
  if (Mode_CV != LastMode_CV) {
    // sum current mode with Mode_CV, wrap value to between 0 and 2
    Mode = mod_floor(ModeFromButton+Mode_CV, 3);
  }
  LastMode_CV = Mode_CV;
